...
template<typename Type> decltype(auto) Entity::getComponent() {
	if (hasComponent<Type>()) { return _registry->get<Type>(_entity); }
	throw TACTICS_EXCEPTION("Can't get component. Entity {} does not have the requested component", getName());
}
...
template<typename Type, typename... Func> decltype(auto) Entity::updateComponent(Func&&... func) {
	if (hasComponent<Type>()) { return _registry->patch<Type>(_entity, std::forward<Func>(func)...); }
	throw TACTICS_EXCEPTION("Can't update component. Entity {} does not have the requested component", getName());
}

template<typename Type> bool Entity::hasComponent() const { return _registry->any_of<Type>(_entity); }

template<typename Type, typename... TArgs> decltype(auto) Entity::addComponent(TArgs&&... args) {
	if (hasComponent<Type>()) { throw TACTICS_EXCEPTION("Entity {} already has the requested component", getName()); }
...
template<typename Type> decltype(auto) Entity::removeComponent() {
	if (!hasComponent<Type>()) {
		throw TACTICS_EXCEPTION("Can't remove component. Entity {} does not have a component of that type", getName());
	}
