...
void GeometryBuilder::addVertex(std::vector<float> vertex) {
	if (!_currentSubMesh) {
		throw Exception("GeometryBuilder: Can't addVertex. No SubMesh currently active. Remember to call beginSubMesh.");
	}
...
void GeometryBuilder::addIndex(unsigned int index) {
	if (!_currentSubMesh) {
		throw Exception("GeometryBuilder: Can't addIndex. No SubMesh currently active. Remember to call beginSubMesh.");
	}
...
void GeometryBuilder::addIndices(std::vector<unsigned int> indices) {
	if (!_currentSubMesh) {
		throw Exception("GeometryBuilder: Can't addIndices. No SubMesh currently active. Remember to call beginSubMesh.");
	}
...
std::shared_ptr<resource::Mesh> GeometryBuilder::build(std::string_view name) {
	if (_currentSubMesh) {
		throw Exception("GeometryBuilder: Can't build mesh. SubMesh is still active. Remember to call endSubMesh.");
	}
	...
	for (auto& subMesh : _subMeshes) {
		if (subMesh.vertices.empty()) {
			throw Exception("GeometryBuilder: Can't build mesh. SubMesh has no vertices.");
		}
		if (subMesh.indices.empty()) {
			throw Exception("GeometryBuilder: Can't build mesh. SubMesh has no indices.");
		}

		auto vbo = std::make_unique<VertexBuffer>(subMesh.vertices);
		auto ibo = std::make_unique<IndexBuffer>(subMesh.indices);
		vbo->bind();
		...
		mesh->subMeshes.emplace_back(
			static_cast<unsigned int>(mesh->subMeshes.size()),
			std::move(vbo),
			std::move(ibo),
			std::move(vao)
		);
