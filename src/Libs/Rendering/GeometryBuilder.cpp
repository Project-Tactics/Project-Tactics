...
void GeometryBuilder::addVertex(std::vector<float> vertex) {
	if (!_currentSubMesh) {
		throw TACTICS_EXCEPTION("GeometryBuilder: Can't addVertex. No SubMesh currently active. Remember to call beginSubMesh.");
	}
...
void GeometryBuilder::addIndex(unsigned int index) {
	if (!_currentSubMesh) {
		throw TACTICS_EXCEPTION("GeometryBuilder: Can't addIndex. No SubMesh currently active. Remember to call beginSubMesh.");
	}
...
void GeometryBuilder::addIndices(std::vector<unsigned int> indices) {
	if (!_currentSubMesh) {
		throw TACTICS_EXCEPTION("GeometryBuilder: Can't addIndices. No SubMesh currently active. Remember to call beginSubMesh.");
	}
