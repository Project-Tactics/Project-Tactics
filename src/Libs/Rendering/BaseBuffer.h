...
public:
	BaseBuffer() { rp::generateBuffers(1, &_id); }
	...
	~BaseBuffer() { rp::deleteBuffers(1, &_id); }

	BaseBuffer(const BaseBuffer&) = delete;
	BaseBuffer& operator=(const BaseBuffer&) = delete;

	void bind() const { rp::bindBuffer(rp::BufferTypeValue<BufferType>::value, _id); }

	void unbind() const { rp::unbindBuffer(rp::BufferTypeValue<BufferType>::value); }
