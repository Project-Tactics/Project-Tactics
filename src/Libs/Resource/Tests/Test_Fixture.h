...
public:
	...
	MOCK_METHOD(std::shared_ptr<MockShaderResource>, load, (const std::string&, const nlohmann::ordered_json&));
};
...
public:
	...
	void expectJsonFileLoad(int times) {
		EXPECT_CALL(*_jsonPackFileHandle, load())
			.Times(Exactly(times));
	}
	...
	void givenJsonFileCreation() {
		...
		EXPECT_CALL(*_fileLoader, createJsonFile(_))
			.WillOnce(Return(ByMove(std::move(_jsonPackFileHandleUniquePtr))));
	}

	void givenPackDefinitionExists(bool exists) {
		EXPECT_CALL(*_jsonPackFileHandle, exists())
			.WillOnce(Return(exists));
	}

	void givenDefaultPackDefinition() {
		_jsonPackFileHandle->setData({
			{"mainPackage", {
				{"shader", {
					{"MyResourceName1", {
	...
	int getNumberOfResources() {
		int count = 0;
		_resourceSystem->forEachResource([&count] (const auto&, const auto&, const auto&) {
			++count;
		});
		return count;
	}
	...
	int getNumberOfLoadedResources() {
		...
		_resourceSystem->forEachManager([&countFromManagers] (const auto& manager) {
			manager.forEachResource([&countFromManagers] (const auto&) {
				++countFromManagers;
			});
		});
	...
	int getNumberOfPacks() {
		int count = 0;
		_resourceSystem->forEachPack([&count] (const Pack&) {
			++count;
		});
		return count;
	}
