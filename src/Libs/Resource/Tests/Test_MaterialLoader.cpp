...
TEST_F(ResourceTest, LoadEmptyMaterialDescriptor) {
	...
	EXPECT_CALL(mockProvider, getResource(ResourceType::Shader, std::string_view("")))
		.Times(0);
...
TEST_F(ResourceTest, LoadMaterialWithValidShader) {
	MockResourceProvider mockProvider;
	EXPECT_CALL(mockProvider, getResource(ResourceType::Shader, "testShader"))
...
TEST_F(ResourceTest, LoadMaterialWithCustomBuiltinUniforms) {
	...
	MaterialDescriptor desc{
		...
		.uniforms = {
			...
			.vectors2 = {{"vec2_1", glm::vec2(0.0f, 1.0f)}, {"vec2_2", glm::vec2(2.0f, 3.0f)}, {"vec2_3", glm::vec2(2.0f, 3.0f)}},
...
TEST_F(ResourceTest, LoadMaterialWithTextureUniforms) {
	...
	EXPECT_CALL(mockProvider, getResource(ResourceType::Texture, "myTexture"))
		.Times(Exactly(2))
	...
	MaterialDescriptor desc{
		.shader = "testShader",
		.uniforms = {
			.textures = {{"texture1", "myTexture"}, {"texture2", "myTexture"}}
	}};
