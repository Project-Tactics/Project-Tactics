...
JsonFileHandle::JsonFileHandle(const std::filesystem::path& path, PathHelper& pathHelper): _path(path), _pathHelper(pathHelper) {
...
void JsonFileHandle::save() {
	if (_path.extension() == ".lua") {
		throw TACTICS_EXCEPTION("Cannot save this json file. Internally it is represented by a lua file. Path: {}", _path.string());
	}
...
void JsonFileHandle::_loadFromLua() {
	...
	try {
		...
		if (!contentFromLua.has_value()) {
			throw TACTICS_EXCEPTION("Failed to load lua file [{}]. Error: Expected string as return value", _path.string());
		}
