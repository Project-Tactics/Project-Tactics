...
template<typename TService> struct WrappedService : public Service {
public:
 ...
 * There's a risk that we might abuse this class by registering any thing just to have some sort of global repository
 * for objects. However, I believe that the risk is low, as the class is not meant to be used as a general purpose
 * repository anyway.
 */
class ServiceLocator {
public:
	template<typename TService> void addService(TService* service) {
	...
	template<typename TService> void removeService(TService*) {
		auto id = ClassId<TService>::ID();
