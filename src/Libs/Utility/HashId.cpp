...
// It's mainly used to check if there are any collisions in the hash generation to avoid surprising bugs which are quite hard to catch.
// In theory we could avoid to use this map in Release builds to avoid the overhead of storing and looking for string but
// it's just better to measure the performance impact before making any decision.
...
[[nodiscard]] const char* HashId::str() const {
	...
	LOG_ERROR(Log::Engine, "Trying to get the string for hash {} but it doesn't exist. Be sure to create all hashed string through the tactics::hash method",
		_id);
	return "";
}
