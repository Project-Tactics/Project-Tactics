...
template<typename T> std::array<T, 4> parseIniStringToVectorValues(const std::string& str) {
	std::array<T, 4> result = {};
	std::stringstream ss(str);
	std::string temp;
	size_t index = 0;

	while (std::getline(ss, temp, ',') && index < result.size()) { ini::Convert<T>().decode(temp, result[index++]); }
